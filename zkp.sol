        ...
        internal view returns (bool)
    {
        ...
        for (uint i = 0; i < proof_inputs.length; i++)
        {
            ...
            assembly {
                ...
                success := staticcall(199000, 7, mul_input, 0x80, add(add_input, 0x40), 0x60)
            }
            require( success );

            assembly {
                // ECADD
                // success := staticcall(199000, 6, mul_input, 0x80, add(add_input, 0x40), 0x60)
                success := staticcall(300000, 6, add_input, 0xc0, add_input, 0x60)
            }
            require( success );
        }
        ...
        uint[1] memory out;
        assembly {
            success := staticcall(300000, 8, input, 768, out, 0x20)
        }
        require(success);
        return out[0] != 0;
    }
